...
- 使用临时 fake Chromium `chrome/app/chrome_main.cc` 已验证 `0001` + `0002` + `0003` + `0004` 可按 series 顺序正常应用，并写入 `MaybeApplyDokeUaClientHints` / `fingerprint.ua_client_hints` / `doke-ua-ch-brands` / `doke-ua-ch-full-version-list`
- 使用临时 fake Chromium `chrome/app/chrome_main.cc` + `components/embedder_support/user_agent_utils.cc` 已验证 `0001` 到 `0005` 可按 series 顺序正常应用，并写入 `GetDokeUserAgentMetadataOverride` / `ReadDokeBrandListSwitch` / `brand_full_version_list`；UA-CH override 不再从 `user_agent_utils.cc` 同步读取 `Doke/runtime.json`，而是读取启动期桥接的 Doke UA-CH switches
...
- 2026-07-18 临时 no-Metal 验证：为验证 Doke patch 编译链路，`third_party/chromium/src/out/Doke/args.gn` 曾临时加入 `angle_enable_metal = false`、`dawn_enable_metal = false`，`buildtools/mac/gn gen out/Doke` 已通过；后续最终链接证明 macOS GPU/WebGPU 路径仍需要 Dawn Metal 符号
- 2026-07-19 第一版可交付构建配置：`out/Doke/args.gn` 当前使用 `angle_enable_metal = false`、`dawn_enable_metal = true`；该配置绕开缺失 Apple MetalToolchain 的 ANGLE Metal 编译路径，同时保留 Dawn Metal 以满足 macOS 链接。它是当前本机可交付配置，不是最终全 Metal/ANGLE Metal 发布配置
- 2026-07-19 构建缺失生成物修复：新增 `tools/ensure_chromium_generated_versions.py` 并接入 `tools/build_doke_chromium.sh`，自动确保 `build/util/LASTCHANGE`、`build/util/LASTCHANGE.committime`、`gpu/webgpu/DAWN_VERSION`、`gpu/webgpu/dawn_commit_hash.h`、`skia/ext/SKIA_VERSION`、`skia/ext/skia_commit_hash.h`、`gpu/config/gpu_lists_version.h`
...
- 2026-07-19 构建脚本修复：`tools/build_doke_chromium.sh` 现在把 Go build cache 放到 `$SRC_DIR/$OUT_DIR/.gocache`，会在构建前运行 `tools/ensure_chromium_generated_versions.py`，并在 `args.gn` 比 `build.ninja` 更新时自动重新 `gn gen`；可用 `DOKE_GO_CACHE=/path/to/cache` 覆盖
- 2026-07-19 第一版真实构建完成：`bash tools/build_doke_chromium.sh` 已完成 `chrome` 目标，产物为 `third_party/chromium/src/out/Doke/Chromium.app`，可执行文件为 `third_party/chromium/src/out/Doke/Chromium.app/Contents/MacOS/Chromium`
- 2026-07-19 第一版验证完成：真实二进制 `python3 tools/doke_probe_check.py /Users/mac/Documents/浏览器/third_party/chromium/src/out/Doke/Chromium.app/Contents/MacOS/Chromium` 通过，输出 `probe_ok version=Doke Chromium 0.1.0` 和 `version_ok Chromium 152.0.7958.0`；Agent `engine.probe` 返回 `available=true`、`probe_protocol=1`；最小 `profile.start` 使用隔离目录 `/private/tmp/doke_real_profile_final` 进入 `running`，启动期间 `tools/doke_runtime_check.py` 校验 `Doke/runtime.json` 通过，`profile.stop` 返回 `stopped`；停止后 runtime-only 文件可能被清理
- 2026-07-19 真实启动修复：首次 `profile.start` 暴露 `GetUserAgentMetadata()` 内同步 `ReadFileToString` 触发 Chromium blocking DCHECK；已将 UA-CH override 改为启动期读取 runtime JSON 并桥接到 Doke UA-CH switches，`components/embedder_support/user_agent_utils.cc` 只读 switches，不再碰文件系统；修复后 Profile 启动 15 秒内未再出现该崩溃
- Smoke Test 对外网弱依赖：即使 `proxy.test` 返回 `ok=false`，只要 Agent/IPC/结果回传链路通，仍会输出 `smoke_ok`
- 当前机器已有第一版真实 Doke Chromium 产物；若要让 UI 默认识别，可设置 `DOKE_CHROMIUM_PATH=/Users/mac/Documents/浏览器/third_party/chromium/src/out/Doke/Chromium.app/Contents/MacOS/Chromium`，或在 Profile 的 Doke Chromium 路径中显式选择该可执行文件。下一步是跑检测基线并决定哪些 native capability 可以从 `capabilities=[]` 晋级
