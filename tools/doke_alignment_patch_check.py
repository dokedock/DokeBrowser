...
def main():
    root = repo_root()
    default_patch = root / "patches" / "chromium" / "0012-doke-runtime-alignment-ingress.patch"
    parser = argparse.ArgumentParser(description="Validate the Doke Chromium alignment ingress patch.")
    ...
    if args.json:
        ...
        if result["ok"]:
            print(f"doke_alignment_patch_ok patch={path}")
        else:
            print("doke_alignment_patch_failed")
            for error in errors:
                print(f"- {error}")
