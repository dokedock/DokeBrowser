...
def main():
    root = repo_root()
    default_patch = root / "patches" / "chromium" / "0015-doke-automation-controlled-policy.patch"
    parser = argparse.ArgumentParser(description="Validate the Doke Chromium AutomationControlled policy patch.")
    ...
    if args.json:
        ...
        if result["ok"]:
            print(f"doke_automation_controlled_patch_ok patch={path}")
        else:
            print("doke_automation_controlled_patch_failed")
            for error in errors:
                print(f"- {error}")
