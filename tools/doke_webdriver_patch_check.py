...
def main():
    root = repo_root()
    default_patch = root / "patches" / "chromium" / "0014-doke-blink-webdriver-policy.patch"
    parser = argparse.ArgumentParser(description="Validate the Doke Chromium webdriver policy patch.")
    ...
    if args.json:
        ...
        if result["ok"]:
            print(f"doke_webdriver_patch_ok patch={path}")
        else:
            print("doke_webdriver_patch_failed")
            for error in errors:
                print(f"- {error}")
